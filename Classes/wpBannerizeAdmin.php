...
class WPBannerizeAdmin extends WPBannerizeClass {
	...
	function saidmadeHeader() {
		?><div class="wp_saidmade_box">
			<p class="wp_saidmade_copy_info"><strong><?php _e('This software is free. You don\'t need to donate money to support it. Just talk about it.', 'wp-bannerize') ?></strong><br/><?php _e('For more info and plugins visit', 'wp-bannerize') ?> <a
	...
	function defaultOptions() {
		$this->options = array(
			'wp_bannerize_version' => $this->version, 'clickCounterEnabled' => '1', 'impressionsEnabled' => '1',
			'supportWPBannerize' => '1', 'comboWindowModeFlash' => 'Window', 'linkDescription' => '0',
			'wpBannerizeStyleDefault' => 'default', 'wpBannerizeStyle' => kWPBannerizeBannerStyleDefault,
	...
	function plugin_admin_scripts() {
		...
		wp_enqueue_script('fancybox_js', $this->uri . kWPBannerizeFancyBoxJavascript,
						  array('jquery'), kWPBannerizeVersion, true);
		wp_enqueue_script('WPBannerizeJavascriptAdmin', $this->uri . kWPBannerizeJavascriptAdmin,
						  array('jquery', 'media-upload', 'thickbox'), kWPBannerizeVersion, true);
		wp_enqueue_script('wp_bannerize_jquery_dp_js', $this->uri . '/js/jquery-ui.min.js',
						  array('jquery'), kWPBannerizeVersion, true);
		wp_enqueue_script('wp_bannerize_timepicker_js', $this->uri . '/js/jquery.timepicker.min.js',
						  array('jquery-ui-core'), kWPBannerizeVersion, true);
	...
	function didToolsLoadPage() {
		add_meta_box('wp_bannerize_tools_editor', __('PHP Function and Shortcode Editor', 'wp-bannerize'),
					 array(&$this, 'boxTools'), $this->pageTools, 'normal', 'core');
		add_meta_box('wp_bannerize_tools_database', __('Database', 'wp-bannerize'),
					 array(&$this, 'boxToolsDatabase'), $this->pageTools, 'side', 'core');
	}

	function didSettingsLoadPage() {
		add_meta_box(kWPBannerizeMetaBoxSettingsKey, __('Settings', 'wp-bannerize'),
					 array(&$this, 'boxSettings'), $this->pageSettings, 'normal', 'core');
	}
	...
	function settings() {
		...
		if (isset($_POST['command_action'])) {
			if ($_POST['command_action'] == "updateSettings" && !isset($_POST['tools'])) {
				...
				$this->options['comboWindowModeFlash'] = (isset($_POST['comboWindowModeFlash']))
						? $_POST['comboWindowModeFlash'] : 'Window';
				$this->options['linkDescription'] = (isset($_POST['linkDescription'])) ? '1' : '0';
				$this->options['wpBannerizeStyleDefault'] = (isset($_POST['wpBannerizeStyleDefault']))
						? $_POST['wpBannerizeStyleDefault'] : 'default';

				$this->options['wpBannerizeStyleCustom'] = (isset($_POST['wpBannerizeStyleCustom']))
						? $_POST['wpBannerizeStyleCustom'] : $this->options['wpBannerizeStyleCustom'];

				$this->options['wpBannerizeStyle'] = (isset($_POST['wpBannerizeStyle'])) ? $_POST['wpBannerizeStyle']
						: kWPBannerizeBannerStyleDefault;

				$this->options['wpBannerizeNoBannerHTMLMessage'] = (isset($_POST['wpBannerizeNoBannerHTMLMessage']))
						? $_POST['wpBannerizeNoBannerHTMLMessage'] : '';
	...
	<div class="wrap">
		...
		<?php if ($any_error != '') : ?>
		<div id="message" class="<?php echo $this->error ? 'error' : 'updated' ?> fade"><p><?php echo $any_error ?></p></div>
		<?php endif; ?>
	...
	function tools() {
		...
		if(isset($_POST[kWPBannerizeFormSender]) && $_POST[kWPBannerizeFormSender] == kWPBannerizeMetaBoxToolsKey) {
			if(isset($_POST[kWPBannerizeFormAction]) && isset($_POST['securityConfirm']) && $_POST[kWPBannerizeFormAction] == kWPBannerizeFormActionTruncateTable) {
				$this->truncateTable();
...
?>
							...
							<table class="form-table wp_bannerize">
								...
								<tr>
									...
									<td>
										...
										<strong><?php _e('Server Date/time'); ?>
								...
								<tr>
									...
									<td>
										<input type="text" name="url" id="url"/> <label
											for="target"><?php _e('Target', 'wp-bannerize')?>:</label> <?php echo $this->get_target_combo() ?>
									</td>
								</tr>
	...
	function show_banners() {
		...
		<?php
		   // Group Actions
			...
			$page_links = paginate_links(array(
											  ...
											  'format' => 'page=wp-bannerize-mainshow', 'total' => $num_pages,
											  'current' => $pagenum));
			?>
		...
		<div class="tablenav">
			...
			<div class="tablenav-pages">
					<span class="displaying-num"><?php printf(__("Showing %s-%s of %s", 'wp-bannerize'), $pagenum, ($count['showing'] > $limit
								? $limit : $count['showing']), $count['showing']) ?></span>
				<?php echo $page_links ?>
			</div>
			<div class="clear"></div>
		</div>
		...
		<table rel="<?php echo $pagenum . "," . $limit ?>" id="wp_bannerize_list" cellspacing="0" class="widefat">
			...
			<tbody>
				<?php
				$alt = 0;
				$sql = sprintf("SELECT *, IF( (`start_date` < NOW() OR `start_date` = '0000-00-00 00:00:00') AND (`end_date` > NOW() OR `end_date` = '0000-00-00 00:00:00') AND (`maximpressions` = 0 OR `impressions` < `maximpressions`), 'enabled', 'disabled' ) AS status FROM %s WHERE %s ORDER BY `sorter`, `group` ASC LIMIT %s,%s", $this->table_bannerize, $where, (($pagenum - 1) * $limit), $limit);
	...
	<td width="100%">
		...
		<p>
			<span class="start_date <?php echo $item->status ?>"><?php echo (
					$this->mysql_date($item->start_date) == '0000-00-00 00:00:00') ? __('Always', 'wp-bannerize')
					: $this->mysql_date($item->start_date) ?></span>
			<span class="end_date <?php echo $item->status ?>"><?php echo (
					$this->mysql_date($item->end_date) == '0000-00-00 00:00:00') ? __('Always', 'wp-bannerize')
					: $this->mysql_date($item->end_date) ?></span>
		</p>
		<?php endif; ?>

		<?php if ($item->url != '') : ?>
		<p class="clear">
			<span class="wpBannerizeURL"><a title="<?php echo $item->url ?>" target="_blank" href="<?php echo $item->url ?>"><?php echo $this->stringCut($item->url) ?></a></span>
		</p>
		<?php endif; ?>
	...
	<div class="inline-edit" style="display:none">
		...
		<fieldset>
			...
			<div <?php hidden($row->banner_type, 1) ?> class="wpBannerizeBannerType1"><input type="file" name="filename" id="filename"/></div>
			<div <?php hidden($row->banner_type, 2) ?> class="wpBannerizeBannerType2"><input value="<?php echo $row->filename ?>"
				...
				<input id="wpBannerizeButtonFromMediaLibrary" type="button" class="button-secondary"
					   value="<?php _e('Media Library Image', 'wp-bannerize')?>"/></div>
			<div <?php hidden($row->banner_type, 3) ?> class="wpBannerizeBannerType3"><textarea name="freeHTML" id="freeHTML" rows="2"
		...
		<p>
			<label for="start_date"><?php _e('Start Date', 'wp-bannerize') ?>:</label> <input class="date" type="text"
																							  name="start_date"
																							  id="start_date" size="18"
																							  value="<?php echo (($row->start_date == "" || $row->start_date == "0000-00-00 00:00:00")
																									  ? ''
																									  : $this->mysql_date($row->start_date)) ?>"/>
			<span class="eraser" onclick="jQuery('input#start_date').val('')"></span>
			<label for="end_date" style="float:none;display:inline;margin-left:16px"><?php _e('End Date', 'wp-bannerize') ?>:</label>
			<input class="date" type="text" name="end_date" id="end_date" size="18"
				   value="<?php echo (($row->end_date == "" || $row->end_date == "0000-00-00 00:00:00") ? ''
						   : $this->mysql_date($row->end_date)) ?>"/> <span class="eraser" onclick="jQuery('input#end_date').val('')"></span> <strong style="color:#888"><?php _e('Server Date/time'); ?>
			: <?php echo date($this->getPHPDateFormat()) ?></strong>
		</p>
		...
		<p>
			...
			<label for="impressions" style="float:none;display:inline;margin-left:16px"><?php _e('Impressions', 'wp-bannerize') ?>:</label>
			<input class="number" type="text" name="impressions" id="iImpressions" value="<?php echo $row->impressions ?>" size="8"/>
			<label for="maxImpressions" style="float:none;display:inline;margin-left:16px"><?php _e('Max Impressions', 'wp-bannerize') ?>:</label>
			<input class="number" type="text" name="maxImpressions" id="maxImpressions" value="<?php echo $row->maximpressions ?>"
				   size="4"/>
		</p>
		...
		<p>
			...
			<input <?php echo (($row->nofollow == '1') ? 'checked="checked"' : '') ?> type="checkbox" name="nofollow"
																					  id="nofollow" value="1"/>
			<label for="width" style="float:none;display:inline;margin-left:16px"><?php _e('Width', 'wp-bannerize') ?>:</label>
			<input size="4" type="text" name="width" id="width" value="<?php echo $row->width ?>"/>
			<label for="height" style="float:none;display:inline;margin-left:16px"><?php _e('Height', 'wp-bannerize') ?>:</label>
		...
		<p class="submit inline-edit-save">
			...
			<a onclick="WPBannerizeJavascript.update(<?php echo $row->id ?>)" class="button-primary save alignright"
			   ...
			   accesskey="s"><?php _e('Update', 'wp-bannerize') ?></a><span style="display:none" class="wpBannerizeAjaxLoader"></span>
		</p>
	</div><?php
		$result = ob_get_contents();
		$result = str_replace("\n", "", $result);
	...
	function combo_group_filter() {
		global $wpdb;
		$o = '<select name="combo_group_filter">' . '<option value="">' . __('All groups', 'wp-bannerize') . '</option>';
	...
	function get_target_combo($sel = "_blank") {
		$o = '<select name="target" id="target">' . '<option></option>' . '<option ' . (($sel == '_blank')
				? 'selected="selected"' : '') . '>_blank</option>' . '<option ' . (($sel == '_parent')
				? 'selected="selected"' : '') . '>_parent</option>' . '<option ' . (($sel == '_self')
				? 'selected="selected"' : '') . '>_self</option>' . '<option ' . (($sel == '_top')
				? 'selected="selected"' : '') . '>_top</option>' . '</select>';
		return $o;
	}
	...
	function addBannerWithFreeHTML() {
		...
		$rows = $wpdb->insert($this->table_bannerize, array(
														   ...
														   'use_description' => $use_description, 'url' => $url,
														   'target' => $target, 'nofollow' => $nofollow,
	...
	function addBannerFromURL() {
		...
		$rows = $wpdb->insert($this->table_bannerize, array(
														   ...
														   'group' => $group, 'description' => $description,
														   'use_description' => $use_description, 'url' => $url,
														   'filename' => $_POST['filenameFromURL'], 'target' => $target,
														   'nofollow' => $nofollow, 'start_date' => $start_date,
														   'end_date' => $end_date,
														   'maximpressions' => $_POST['maxImpressions'],
														   'mime' => $mime, 'width' => $dimensions[0],
														   'height' => $dimensions[1]
		...
		if (is_uploaded_file($_FILES['filename']['tmp_name'])) {
			...
			if (move_uploaded_file($temp, $uploads['file'])) {
				...
				$rows = $wpdb->insert($this->table_bannerize, array(
																   ...
																   'group' => $group, 'description' => $description,
																   'use_description' => $use_description, 'url' => $url,
																   'filename' => $uploads['url'], 'target' => $target,
																   'nofollow' => $nofollow, 'mime' => $mime,
	...
	function deleteBanner($id = null) {
		...
		$wpBannerizeBannerType = $wpdb->get_var("SELECT `banner_type` FROM `" . $this->table_bannerize . "` WHERE `id` = " . $id);
	...
	function updateBannerWithFreeHTML() {
		...
		$values = array(
			'banner_type' => $_POST['wpBannerizeBannerType'], 'group' => $_POST['group'],
			'start_date' => $this->mysql_date($_POST['start_date']),
	...
	function stringCut($s, $l = 32, $f = "...") {
		if (strlen($s) > $l)
			return substr($s, 0, ($l - strlen($f)) / 2) . $f . substr($s, -($l - strlen($f)) / 2, ($l - strlen($f)) / 2); else return $s;
	}
	...
	function _iterateCategory($cat_object, $selected = null) {
		...
		$ou = '<li><label><input ' . $checked . ' type="checkbox" name="wpBannerizeCategoriesTree[]" class="wpBannerizeCategoriesTree" value="' .
