...
class WPBannerizeFrontend extends WPBannerizeClass {
	...
	function WPBannerizeFrontend($__file__) {
		...
		wp_localize_script('wp_bannerize_frontend_js', 'wpBannerizeJavascriptLocalization',
						   array('ajaxURL' => $this->ajaxURL));
				...
				if ($row->banner_type == kWPBannerizeBannerTypeFromLocal || $row->banner_type == kWPBannerizeBannerTypeByURL) :
					...
					<?php else:	?>
						<?php if( $args['no_html_wrap'] == '' ) echo $args['before']; ?>
							...
							<a <?php echo $javascriptClickCounter ?> href="<?php echo $row->url ?>" <?php echo ( ($row->nofollow == '1') ? 'rel="nofollow"' : '')  ?> <?php echo (  ($row->target != '') ? 'target="' . $row->target . '"' : '' )  ?>>
							<?php endif; ?>
								<img src="<?php echo $row->filename ?>" alt="<?php echo $row->description ?>" <?php echo ( ($row->width == 0 || $row->height == 0) ? '' : sprintf('width="%s" height="%s"', $row->width, $row->height) ); ?>/>
							<?php if($row->url != '') : ?>
							</a>
							<?php endif; ?>
							<?php if ($row->use_description == '1') : ?>
