<form class="wpBannerizeForm" name="save_settings" method="post" action="">
	...
	<table class="form-table wp_bannerize">
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="linkDescription" id="linkDescription"
					   value="1" <?php echo ($this->options['linkDescription'] == "1") ? 'checked="checked"' : '' ?> />
			</td>
		</tr>
		...
		<tr>
			<td colspan="2">
				<textarea <?php echo ($this->options['wpBannerizeStyleDefault'] == 'default') ? 'disabled="disabled"'
						: '' ?> class="<?php echo ($this->options['wpBannerizeStyleDefault'] == 'default') ? 'disabled'	: '' ?>" rows="3" cols="20" name="wpBannerizeStyleCustom"
