...
class WPBANNERIZE {
	...
	function WPBANNERIZE() {
		...
		$this->_PLUGINNAME				= "wp-bannerize";
		$this->_OPTIONSKEY				= "wp-bannerize";
		$this->_OPTIONSTITLE			= "wp-bannerize";
		
		$this->_UPLOADS_URL				= get_option('siteurl') . '/wp-content/uploads/';
		$this->_UPLOADS_PATH			= realpath( dirname(__FILE__) . '/../../uploads/' ) . '/';
		$this->_AJAX_URL				= get_option('siteurl') .  "/wp-content/plugins/wp-bannerize/ajax.php";
		
		$this->_TABLE_BANNERIZE			= "bannerize";
	...
	function init() {
		 ...
		 */
		add_option( $this->_OPTIONSKEY, $this->options, $this->_OPTIONSTITLE );
		
		/**
		 * re-Load options
		 */
		$this->options = get_option( $this->_OPTIONSKEY );
	...
	function set_options_page() {
		if ( function_exists('add_options_page') ) {
	 		$plugin_page = add_options_page( $this->_OPTIONSTITLE, $this->_OPTIONSTITLE, 8, basename(__FILE__), array( $this, 'set_options_subpanel') );
	...
	function set_options_subpanel() {
		...
		<?php
			...
			$q = "SELECT * FROM `" . $this->_TABLE_BANNERIZE . "`";
			...
			foreach( $rows as $row ) {
				$o .= '<tr id="item_' . $row->id . '">' .
					  ...
					  '<td align="center"><a target="_blank" href="' . $row->url . '"><img height="55" border="0" src="' . $this->_UPLOADS_URL . $row->filename . '" /></a></td>' .
	...
	function combo_group() {
		...
		$q = "SELECT `group` FROM `" . $this->_TABLE_BANNERIZE . "` GROUP BY `group` ORDER BY `group` ";
		$rows = $wpdb->get_results( $q );
		$sel = "";
		foreach( $rows as $row ) {
	...
	function mysql_insert() {
		...
		if( $_FILES['filename']['error'] == 0 ) {
			...
			$pathname = $this->_UPLOADS_PATH . $basepath;
			...
			if ( move_uploaded_file( $_FILES['filename']['tmp_name'], $filename )) {
				...
				$q = "INSERT INTO `" . $this->_TABLE_BANNERIZE . "`" .
	...
	function mysql_delete() {
		...
		//
		$filename = $wpdb->get_var( "SELECT filename FROM `" . $this->_TABLE_BANNERIZE . "` WHERE `id` = " . $_POST['id'] );
		@unlink( $this->_UPLOADS_PATH . $filename );
		
		$q = "DELETE FROM `" . $this->_TABLE_BANNERIZE . "` WHERE `id` = " . $_POST['id'];
		$wpdb->query($q);
		return('');
	}
	...
	function checkTable() {
		...
		$q = 'CREATE TABLE IF NOT EXISTS `' . $this->_TABLE_BANNERIZE . '` (
	...
	function bannerize( $args = '' ) {
		...
		foreach( $rows as $row ) {
			$o .= $new_args['before'] . 
				  '<a target="_blank" href="' . $row->url . '"><img border="0" src="' . $this->_UPLOADS_URL . $row->filename . '" /></a>' .
				  $new_args['after'];	
		}
...
$wpbannerize = new WPBANNERIZE();

function wp_bannerize( $args = '' ) {
	global $wpbannerize;
	$wpbannerize->bannerize( $args );
}
